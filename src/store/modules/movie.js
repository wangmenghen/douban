...
const actions = {
  async [GET_MOVIE] ({ commit, state }, payload) {
    try {
      let hotMovie = await getMovie('in_theaters', payload.count)
      commit({
        type: `${GET_MOVIE}`,
        tag: 'hotMovie',
        res: hotMovie.data.subjects
      })
      let topMovies = await getMovie('top250', payload.count)
      commit({
        type: `${GET_MOVIE}`,
        tag: 'topMovies',
        res: topMovies.data.subjects
      })
      let newMovies = await getMovie('coming_soon', payload.count)
      commit({
        type: `${GET_MOVIE}`,
        tag: 'newMovies',
        res: newMovies.data.subjects
      })
    } catch (e) {
      throw e
    }
